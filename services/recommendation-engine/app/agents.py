...
"""
Legacy pre-LLM Brain agent implementation retained only as non-executable
history for older imports. Active callers are re-exported from agent_runtime
below, where recommendations are generated by the LLM Brain instead of a
curated tool list.

The old code block is intentionally inside this module docstring so it cannot
run or import the retired knowledgebase.
...
# Compatibility layer:
# Keep app.agents importable while routing callers to the LLM Brain runtime.
